...
- отдельным Git-репозиторием, если вы заранее проверили его на секреты и личные
  данные.
...
Экран **Learning** появляется без изменения обычного поиска, если в vault есть
`_meta/LEARNING_CATALOG.md` с fenced JSON-блоком `learning-catalog`. Каталог
связывает навыки с существующими Markdown-заметками, содержит diagnostics и
показывает происхождение необязательного roadmap.

После изменения каталога запустите:

```bash
python -m app.learning_catalog --vault /absolute/path/to/vault
```

На Windows путь передаётся тем же способом, например:

```powershell
.\.venv\Scripts\python.exe -m app.learning_catalog --vault C:\Users\Ilya\Documents\My_brain_v2
```

Каталог имеет `rag: exclude`, поэтому служебный JSON не попадает в ответы агента.
Личная память обучения никогда не записывается в vault: она находится в
