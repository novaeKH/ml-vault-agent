...
def discover_vaults() -> list[str]:
    ...
    # Windows commonly redirects Documents/Desktop into OneDrive.
    for variable in ("OneDrive", "OneDriveConsumer", "OneDriveCommercial"):
        value = os.environ.get(variable, "").strip()
        if value:
            one_drive = Path(value).expanduser()
            search_roots.extend(
                [one_drive, one_drive / "Documents", one_drive / "Desktop"]
            )
    ...
    return [
        str(path)
        for path in sorted(found, key=lambda item: _candidate_score(item), reverse=True)
    ]
...
def load_settings() -> Settings:
    ...
    if not settings.vault_path or not _is_vault(Path(settings.vault_path)):
        ...
        if len(candidates) == 1:
            ...
            first_score = _candidate_score(Path(candidates[0]))
            second_score = _candidate_score(Path(candidates[1]))
            unique_best = first_score > second_score
